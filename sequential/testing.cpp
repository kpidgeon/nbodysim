...
Point3D randomPoint(Point3D bound1, Point3D bound2){

    // initiate Mersenne Twister RNG and seed
    ...
    float randX = distrX(mtGen);
    float randY = distrY(mtGen);
    float randZ = distrZ(mtGen);
...
std::vector<GravObj> genUniformEnsemble(int n, Point3D bound1, Point3D bound2){
    ...
    for(int i=0; i<n; i++){
        ...
        GravObj p(randomPoint(bound1, bound2), 0, 0, 0, 10);
...
int main(){
    ...
    // std::vector<GravObj> ensemble = genUniformEnsemble(10, lowBound, highBound);
